...
detector_dir = Path(r"C:\Users\filip\PycharmProjects\SejmBot\data_sejm\detector")
files = sorted(detector_dir.glob('*.json'), key=lambda p: p.stat().st_mtime, reverse=True)
if not files:
    print('No detector result files found in', detector_dir)
    raise SystemExit(1)
